...
CASE_DIR = Path(__file__).parent / "facebook_recipe_cases"
...
def load_case(path: Path) -> dict[str, object]:
    ...
    if case["platform"] != "facebook" or case["source_text_kind"] != "description":
        raise ValueError("cases must use a Facebook description")
    if (
        not isinstance(case["queue_index"], int)
        or isinstance(case["queue_index"], bool)
        or not 1 <= case["queue_index"] <= 12
        or not isinstance(case["source_text"], str)
        or not case["source_text"].strip()
    ):
        raise ValueError("queue_index must be 1..12 and source_text must be nonblank")
    for field in ("source_url", "resolved_source_url", "source_name"):
        if not isinstance(case[field], str) or not case[field]:
            raise ValueError(f"{field} must be nonblank text")
    for field in ("creator", "thumbnail_url"):
    ...
    if not isinstance(case["expected_complete"], bool) or not isinstance(
        case["accepted"], bool
    ):
    ...
    if case["expected_complete"] != evidence_is_complete:
        raise ValueError("expected_complete must agree with the labeled evidence")
    return case
...
def main() -> int:
    ...
    for path in case_paths:
        try:
            ...
            if case["accepted"] and not correct:
                status += " REGRESSION"
                failures += 1
            elif not case["accepted"] and correct:
    ...
    coverage = correct_complete / source_complete if source_complete else 1.0
    precision = correct_complete / parser_complete if parser_complete else 1.0
